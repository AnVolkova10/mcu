    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl sm:text-4xl font-black text-white tracking-wider flex items-center gap-3 font-title uppercase">
            ...
            <span>SAVED & BOOKMARKED EVENTS ({bookmarkedEvents.length})</span>
          </h1>
          <p className="text-sm text-zinc-400 mt-1 font-din">
            Your personalized collection of pivotal MCU moments and milestones saved for quick access.
          </p>
        </div>

        <button
          onClick={() => setActiveScreen('timeline')}
          className="flex items-center gap-2 px-4 py-2 rounded bg-[#141414] hover:bg-[#e62429] text-white text-xs font-bold font-title tracking-wider uppercase border border-[#2e2e2e] hover:border-[#e62429] transition-all cursor-pointer"
        >
      ...
      {bookmarkedEvents.length === 0 ? (
        <div className="text-center py-20 bg-[#141414] border border-[#27272a] rounded-xl p-8">
          ...
          <h3 className="text-xl font-bold text-white mb-1 font-title uppercase">No saved events yet</h3>
          <p className="text-sm text-zinc-400 max-w-md mx-auto mb-5 font-din">
            Click the bookmark icon on any timeline card to save historical events here.
          </p>
          <button
            onClick={() => setActiveScreen('timeline')}
            className="px-4 py-2 rounded bg-[#e62429] hover:bg-[#c5191e] text-white text-xs font-bold font-title tracking-wider uppercase transition-all shadow-lg cursor-pointer"
          >
            EXPLORE TIMELINE
          </button>
        </div>
          ...
          {bookmarkedEvents.map(({ event, eraTitle }) => (
            <div key={event.id} className="relative">
              <div className="text-xs font-bold text-[#e62429] mb-2 flex items-center gap-1.5 font-title tracking-wider uppercase">
                ...
                <span>TIMELINE ERA: {eraTitle}</span>
              </div>
